...
var Settings = class {
    constructor(schemas) {
        ...
        for (const key in schemas) {
            ...
            if (schemaObj) {
                ...
                helper.warn(`Schema ${key} (${schema}) not found`);
            }
        }
    }
    ...
    get(key, name) {
        ...
        if (!settings) {
            return helper.warn(`Cannot find settings ${key}`);
        }
        ...
        switch (variantType) {
            ...
            case 's':
                const value = variant.get_string();
    ...
    set(key, name, value) {
        ...
        if (!settings) {
            return helper.warn(`Cannot set settings ${name} with ${value}`);
        }
    ...
    reset(key, name) {
        const settings = this.get(key);
        if (!settings) {
            return helper.warn(`Cannot reset settings ${name}`);
        }
    ...
    track(key, name, callback, notCallback) {
        const settings = this.get(key);
        if (!settings) {
            return helper.warn(`Cannot track settings ${name}`);
        }
